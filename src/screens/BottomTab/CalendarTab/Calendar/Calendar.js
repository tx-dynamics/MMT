...
const Calendars = props => {
  ...
  async function getTrakee() {
    ...
    data.on('value', childern => {
        ...
        arr.push({
          ...
          lastDate: new Date(dat?.lastDate).getFullYear() < new Date().getFullYear() ?
            moment(dat?.lastDate).add({
              month: new Date().getMonth() - new Date(dat?.lastDate).getMonth(),
      ...
      let markedDay = {};
      if (arr.length > 0) {
        const sd = (moment(arr[0].lastDate).add(14, 'day'));
        const ft = (moment(arr[0].lastDate).add(14, 'day'));
        console.log("Fertility", ft)
        ...
        setTimeout(() => {
          ...
          console.log("Cycle Setting", arr[0]?.cycle)
        }, 300);
  ...
  const setFertsMethod = (i) => {
    const ft = (moment(Relationship[i]?.lastDate).add(14, 'day'));
    console.log("ft", ft)
    setfert(moment(ft).format('yyy-MM-DD'))
  }

  // const markPeriods = (item) =>{
  // console.log("item", item)
  // let markedDay={}
  // const prds =(moment(item.lastDate).add(5,'day'));

  // for(let i=1;i<5;i++) {
  //   const prdss =(moment(prds).add(i,'day'));
  //   markedDay[moment(prdss).format('YYYY-MM-DD')] = {selected: true, selectedColor: 'blue',}  
  // }
  // setmarkedDays(markedDay)
  // console.log("prds", markedDay)
  // }
  ...
  function seletedval(value) {
    ...
    Relationship.map(items => {
      if (items.uid === value) {
        ...
        setname(items.name);
        const sd = (moment(items?.lastDate).add(14, 'day'));
        // const prds = (moment(items?.lastDate).add(items.cycle, 'day'));
        // console.log("ckkkkkkkkkk",prds, items.cycle, moment(items.lastDate).format("DD"))
        // let a =Number(items.cycle) - 5
        // let b = Number(items.cycle)+1 
        // console.log("a, b", a,b)
        // for (let i = a; i < b ; i++) {
        //   const prdss = (moment(prds).add(i, 'day'));
        //   markedDay[moment(prdss).format('YYYY-MM-DD')] = { selected: true, selectedColor: 'blue', }
        // }
        // console.log("periods Marking", markedDay)
        for (let i = 10; i < 15; i++) {
        ...
        // console.log("Dusra Marking", markedDay)
        setmarkedDays(markedDay);
        setlastday(moment(sd).format('yyy-MM-DD'));
      }
    })
    ...
    <TouchableOpacity
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', width: '90%', paddingHorizontal: 5, }}>
        ...
        <Text numberOfLines={1} style={{
          fontWeight: '400', fontFamily: Fonts.Poppins, fontSize: 13, marginLeft: 15,
          color: '#383838'
        }}>{item.name}
        </Text>
      </View>
            ...
            return (
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                {Number(cycle) === date?.day ?
                  <Image source={play} resizeMode='contain' style={{
                    height: 10.37, width: 10.91,
                ...
                {Number(cycle) === date?.day ?
                  <Image source={line} resizeMode='contain' style={{
                ...
                >
                  <Text style={{
                    textAlign: 'center', color: state === 'disabled' ? 'transparent' : marking ? 'tomato' : '#383838',
